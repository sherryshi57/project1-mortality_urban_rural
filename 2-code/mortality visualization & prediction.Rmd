...
```{r-wrong prediction}
library(broom)  # For easy handling of model outputs
library(purrr)  # Ensure purrr is loaded



# Fit linear models for each PopSize, storing each model in a list
models <- data_filtered %>%
  group_by(PopSize) %>%
  summarize(model = list(lm(Value ~ Year, data = .)), .groups = 'drop')

# Prepare to predict values for the next 5 years
years_to_predict <- data.frame(Year = seq(2023, 2028))

# Predict using the models and add the Year column appropriately
predictions <- models %>%
  mutate(predictions = map(model, ~predict(.x, newdata = years_to_predict))) %>%
  select(-model) %>%
  unnest(predictions) %>%
  rename(Value = predictions) %>%
  mutate(PopSize = rep(unique(data_filtered$PopSize), each = nrow(years_to_predict)),
         Year = rep(years_to_predict$Year, times = n_distinct(data_filtered$PopSize)))

# Combine actual data and predictions
combined_data <- bind_rows(data_filtered, predictions)

# Plotting with actual data and predictions
plot <- ggplot(combined_data, aes(x = Year, y = Value, color = PopSize)) +
  geom_line() +
  geom_point(data = data_filtered) +
  geom_point(data = predictions, shape = 2) +  # Use a different shape for predictions
  scale_color_manual(values = c("Large Metro" = "red", "Small/Medium Metro" = "blue", "Rural" = "orange")) +
  labs(title = "Projected Age-adjusted Mortality Rates by Rurality",
       x = "Year",
       y = "Mortality Rate (per 100,000)") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_x_continuous(breaks = seq(min(data_filtered$Year), max(years_to_predict$Year), by = 1)) +
